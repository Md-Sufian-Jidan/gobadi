...
exports.deleteFarmer = exports.getFarmerById = exports.deleteDoctor = exports.getDoctorById = exports.getDoctorsList = exports.getFarmersList = exports.getRetention = exports.getTaskFeatureUsers = exports.getRegisteredAnimals = exports.getDailyUsers = exports.getUserLocation = exports.getUserListStats = exports.getAppointments = exports.getAiUsers = exports.getUserOS = exports.getUserGrowth = exports.getDashboardStats = void 0;
...
function getDayLabel(date, format) {
    if (format === "day") {
...
const getDashboardStats = async (period) => {
    ...
    const [totalUsers, totalFarmers, totalDoctors, currentPeriodUsers, previousPeriodUsers,] = await Promise.all([
    ...
    const changePercent = previousPeriodUsers > 0
        ? Math.round(((currentPeriodUsers - previousPeriodUsers) / previousPeriodUsers) *
            100)
...
const getUserGrowth = async (period) => {
    ...
    const { start: prevStart, end: prevEnd } = getPreviousDateRange(period);
    const prevUsers = await prisma_1.prisma.user.count({
        where: { createdAt: { gte: prevStart, lte: prevEnd } },
    });
...
// ─── User OS Chart ────────────────────────────────────────────────────────────
const getUserOS = async (period) => {
    const { start, end } = getDateRange(period);
    const format = getGroupByFormat(period);
    const pushTokens = await prisma_1.prisma.pushToken.findMany({
        where: { createdAt: { gte: start, lte: end } },
        select: { os: true, createdAt: true },
        orderBy: { createdAt: "asc" },
    });
    // Initialize grouped data
    const androidGrouped = {};
    const iosGrouped = {};
    if (format === "day") {
        const current = new Date(start);
        while (current <= end) {
...
exports.getUserOS = getUserOS;
// ─── AI Users ─────────────────────────────────────────────────────────────────
const getAiUsers = async (period) => {
    const { start, end } = getDateRange(period);
    const { start: prevStart, end: prevEnd } = getPreviousDateRange(period);
    const [currentCount, previousCount, totalAiUsers, totalUsers] = await Promise.all([
        prisma_1.prisma.aiDiagnosis.findMany({
            where: { createdAt: { gte: start, lte: end } },
            select: { userId: true },
        }),
        prisma_1.prisma.aiDiagnosis.findMany({
    ...
    const changePercent = previousUnique > 0
        ? Math.round(((currentUnique - previousUnique) / previousUnique) * 100)
        : currentUnique > 0
            ? 100
            : 0;
    const gaugePercentage = totalUsers > 0 ? Math.round((totalUnique / totalUsers) * 100) : 0;
    return {
...
exports.getAiUsers = getAiUsers;
// ─── Doctor's Appointments ────────────────────────────────────────────────────
const getAppointments = async (period) => {
    const { start, end } = getDateRange(period);
    const format = getGroupByFormat(period);
...
// ─── User List Stats ──────────────────────────────────────────────────────────
const getUserListStats = async (period) => {
    const { start, end } = getDateRange(period);
    const { start: prevStart, end: prevEnd } = getPreviousDateRange(period);
    const [totalFarmers, totalDoctors, totalActiveAnimals, totalReferralUsers, prevFarmers, prevDoctors, prevActiveAnimals, prevReferralUsers,] = await Promise.all([
        prisma_1.prisma.user.count({ where: { role: "user" } }),
        prisma_1.prisma.user.count({ where: { role: "doctor" } }),
        prisma_1.prisma.animal.count(),
        prisma_1.prisma.referral.count({ where: { referralCount: { gt: 0 } } }),
...
exports.getUserListStats = getUserListStats;
// ─── User Location Chart ──────────────────────────────────────────────────────
const getUserLocation = async (period, role, filter) => {
    const locationField = filter === "Division"
        ? "division"
        : filter === "Upazila"
            ? "upazila"
            : "district";
...
// ─── Daily Users Chart ────────────────────────────────────────────────────────
const getDailyUsers = async (period) => {
    const { start, end } = getDateRange(period);
    const { start: prevStart, end: prevEnd } = getPreviousDateRange(period);
    const format = getGroupByFormat(period);
    const users = await prisma_1.prisma.user.findMany({
...
const getRegisteredAnimals = async (period) => {
    const { start, end } = getDateRange(period);
    const { start: prevStart, end: prevEnd } = getPreviousDateRange(period);
    // Animals don't have createdAt in NestJS entity, count all
    const totalAnimals = await prisma_1.prisma.animal.count();
...
exports.getRegisteredAnimals = getRegisteredAnimals;
// ─── Task Feature Users Chart ─────────────────────────────────────────────────
const getTaskFeatureUsers = async (period) => {
    const { start, end } = getDateRange(period);
    const { start: prevStart, end: prevEnd } = getPreviousDateRange(period);
    ...
    const userSeenPerPeriod = new Map();
    tasks.forEach((task) => {
        let key;
        if (period === "last_7_days" || period === "last_30_days") {
    ...
    const prevTasks = await prisma_1.prisma.task.findMany({
        where: { createdAt: { gte: prevStart, lte: prevEnd } },
        select: { userId: true },
    });
...
exports.getTaskFeatureUsers = getTaskFeatureUsers;
// ─── User Retention ───────────────────────────────────────────────────────────
const getRetention = async (period) => {
    const { start, end } = getDateRange(period);
    const { start: prevStart, end: prevEnd } = getPreviousDateRange(period);
    const format = getGroupByFormat(period);
    // Get refresh tokens created in the period (represents active sessions)
    const refreshTokens = await prisma_1.prisma.refreshToken.findMany({
        where: { createdAt: { gte: start, lte: end } },
        select: { userId: true, createdAt: true },
        orderBy: { createdAt: "asc" },
    });
    ...
    const prevRefreshTokens = await prisma_1.prisma.refreshToken.findMany({
        where: { createdAt: { gte: prevStart, lte: prevEnd } },
        select: { userId: true },
    });
    ...
    const targetLine = allValues.length > 0
...
// ─── Farmers List ─────────────────────────────────────────────────────────────
const getFarmersList = async (page, limit, search, status) => {
...
exports.getFarmersList = getFarmersList;
// ─── Doctors List ─────────────────────────────────────────────────────────────
const getDoctorsList = async (page, limit, search, status) => {
    const skip = (page - 1) * limit;
